...
float Model_3DS::GetWidth() {
	return bounds.getWidth();
}

float Model_3DS::GetHeight() {
	return bounds.getHeight();
}

float Model_3DS::GetDepth() {
	return bounds.getDepth();
}

// Implementation of the new boundary checking methods
void Model_3DS::CalculateBoundingBox() {
	if (numObjects == 0) return;

	// Initialize bounds with first vertex
	bounds.minX = bounds.maxX = Objects[0].Vertexes[0];
	bounds.minY = bounds.maxY = Objects[0].Vertexes[1];
	bounds.minZ = bounds.maxZ = Objects[0].Vertexes[2];

	// Loop through all objects and vertices to find min/max values
	for (int i = 0; i < numObjects; i++) {
		for (int v = 0; v < Objects[i].numVerts * 3; v += 3) {
			// Apply model transformation
			float x = Objects[i].Vertexes[v] * scale + pos.x;
			float y = Objects[i].Vertexes[v + 1] * scale + pos.y;
			float z = Objects[i].Vertexes[v + 2] * scale + pos.z;
